...
func (g *Gossiper) HandleRumor(wrapped_pkt *message.PacketIncome) {
	...
	if updated {
		...
		g.MongerRumor(rumor, false, sender)
	}
...
func (g *Gossiper) MongerRumor(rumor *message.RumorMessage, flipped bool, excluded ...string) {
...
func (gossiper *Gossiper) HandleSimple(wrapped_pkt *message.PacketIncome) {
	...
	gossiper.MongerRumor(gossiper.RumorBuffer.Rumors[gossiper.Name][len(gossiper.RumorBuffer.Rumors[gossiper.Name]) - 1], false)
}


// Handle timeout resend
func (g *Gossiper) HandleRumorMongeringTimeout() {

	go func() {
		for timeout_pkt := range g.N.RumorTimeoutCh {

			g.FlipCoinMonger(timeout_pkt.Packet.Rumor, timeout_pkt.Addr)
		}
	} ()
...
func (g *Gossiper) FlipCoinMonger(rumor *message.RumorMessage, excluded ...string) {
	...
	if continue_monger == 0 {
		...
		g.MongerRumor(rumor, true, excluded[0])
	}
}